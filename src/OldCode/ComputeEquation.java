...
public class ComputeEquation {
	...
	private void getDocSimDic() {
		try {
			BufferedReader br = new BufferedReader(
					new FileReader(
							"/home/archana/SCU_projects/research_changes/lucene/doc_similarity.txt"));
			String line = null;
			Set doc_pair = null;
	...
	private void openAreaIndex() {
		try {
			File path = new File(
					"/home/archana/SCU_projects/research_changes/lucene/ResIntAreaIndex");
	...
	private void openYearIndex() {
		try {
			File path = new File(
					"/home/archana/SCU_projects/research_changes/lucene/ResIntYearIndex");
	...
	private BufferedReader openQueryFile() {
		...
		try {
			br = new BufferedReader(
					new FileReader(
							"/home/archana/SCU_projects/research_changes/lucene/res_qry_by_year.json"));
			return br;
		} catch (IOException e) {
			e.printStackTrace();
			return br;
	...
	public static void main(String args[]) throws IOException, ParseException {
		...
		ComputeEquation resAreaScore = new ComputeEquation();
		resAreaScore.Init();
		...
		int totFreqTest = 0;
		totFreqTest = resAreaScore
				.calculateTotTestTeFreq(resAreaScore.testCorpusMap);
		System.out.println("TotalFreqTest = " + totFreqTest);
		...
		BufferedWriter bw1 = new BufferedWriter(
				new FileWriter(
						"/home/archana/SCU_projects/research_changes/lucene/regular_probabilities"));
		BufferedWriter bw2 = new BufferedWriter(
				new FileWriter(
						"/home/archana/SCU_projects/research_changes/lucene/baseline_probabilities"));
		try {
			while ((line = br.readLine()) != null) {
				...
				if (rawQueryStr != null) {
					...
					if (escaped_query_Kt == null) {
						System.out.println(researcher_name
								+ " does not have papers for year " + year);
						Thread.sleep(100);
						continue;
					}
					...
					int totAreas = resAreaScore.areaireader.maxDoc();
					ScoreDoc[] hits_Kt = resAreaScore.areaisearcher.search(
							query_Kt, null, totAreas).scoreDocs;
					...
					final long researcherLoopStartTime = System
							.currentTimeMillis();
					...
					for (int i = 0; i < hits_Kt.length; i++) {
						...
						int second_loop_cnt = 0;
						for (Object doc_set : resAreaScore.doc_sim_dic.keySet()) {
							final long secondLoopStartTime = System
									.currentTimeMillis();
							Set doc_set1 = (Set) doc_set;
							String docno = "" + hits_Kt[i].doc;
							if (doc_set1.contains(docno)) {
								...
								resAreaScore.getAreaTerms(areaname_Ktplus,
										resAreaScore.areaCorpusMap);
								totFreq = resAreaScore
										.calculateTotFreq(resAreaScore.areaCorpusMap);
								// System.out.println("AreaCorpusTerms = " +
								// resAreaScore.areaCorpusMap);
								// System.out.println("TestCorpusTerms = " +
								// resAreaScore.testCorpusMap);
								// System.exit(1);
								double secondProb = val / cummKtPlusProb;
//								final long thirdLoopStartTime = System
//										.currentTimeMillis();
								for (Object term : resAreaScore.testCorpusMap
										.keySet()) {
//									System.out.println("Term = " + term);
									TestTermParams ttp = resAreaScore.testCorpusMap
											.get(term);
									int word_freq = ttp.freq;
									double thirdProb = ttp.prob;
									// Object look_term =
									// resAreaScore.areaCorpusMap.get(term);
									int area_term_freq = 0;
									if (resAreaScore.areaCorpusMap
											.containsKey(term)) {
										area_term_freq = resAreaScore.areaCorpusMap
												.get(term);
									}
									// int look_term =
									// resAreaScore.areaCorpusMap.get(term);
									...
									// int look_term =
									// resAreaScore.areaCorpusMap.get(term);
									// did not consider tf of the word earlier
									// if (look_term != null) {
									...
									if (area_term_freq != 0) {
										thirdProb = alpha * area_term_freq
												/ totFreq;
									} else {
										thirdProb = (1 - alpha) * 1 / totFreq;
									}
									thirdProb *= word_freq;
									double cummProb = firstProb * secondProb
											* thirdProb;
									bw1.write("Kt = " + areaname_Kt
											+ "\tKtPlus = " + areaname_Ktplus
											+ "\tword = " + term + "\n");
									bw1.write("P(Kt/r) = " + firstProb
											+ "\tP(Kt+1/Kt) = " + secondProb
											+ "\tP(w/Kt+1) = " + thirdProb
											+ "\tcummProb = " + cummProb + "\n");
									ttp.prob += cummProb;
//									final long thirdLoopFinishTime = System
//											.currentTimeMillis();
//									System.out
//											.println("Total execution time for third loop: "
//													+ (int) ((thirdLoopFinishTime - thirdLoopStartTime) / 1000)
//													% 60);
								}
//								final long secondLoopFinishTime = System.currentTimeMillis();
//								second_loop_cnt++;
//								System.out.println(second_loop_cnt+" Total execution time for second loop(ms): "
//												+ (int) ((secondLoopFinishTime - secondLoopStartTime) ));
							}
						}
						// System.out.println("Done with second loop");
						final long firstLoopFinishTime = System
								.currentTimeMillis();
						System.out.println("***********************************");
						System.out
								.println("Total execution time for first loop(sec): "
										+ ((firstLoopFinishTime - firstLoopStartTime) / 1000));
						System.out.println("***********************************");
						
					}
					final long researcherLoopFinishTime = System.currentTimeMillis();
					System.out
							.println("Total execution time for researcher loop(min): "
									+ (int)(((researcherLoopFinishTime - researcherLoopStartTime) / 1000)/60));
					...
					System.out.println("Total execution time: "
							+ (endTime - startTime));
					System.out.print("Total probability of the researcher:"
							+ researcher_name + " is " + researcherProb);
					...
					for (int i = 0; i < hits_Kt.length; i++) {
						...
						System.out
								.println("Computing baseline prob for area = "
										+ areaname_Kt);
						int totFreq = 0;
						resAreaScore.getAreaTerms(areaname_Kt,
								resAreaScore.areaCorpusMapBaseline);
						totFreq = resAreaScore
								.calculateTotFreq(resAreaScore.areaCorpusMapBaseline);

						for (Object term : resAreaScore.testCorpusMapBaseline
								.keySet()) {
							TestTermParams ttp = resAreaScore.testCorpusMapBaseline
									.get(term);
							int word_freq = ttp.freq;
							double thirdProb = ttp.prob;
							Object look_term = resAreaScore.areaCorpusMapBaseline
									.get(term);
							if (look_term != null) {
								thirdProb = alpha * 1 / totFreq;
							} else {
							...
							bw2.write("Kt = " + areaname_Kt + "\tKtPlus = "
									+ term + "\n");
							bw2.write("P(Kt/r) = " + firstProb
									+ "\tP(w/Kt+1) = " + thirdProb
									+ "\tcummProb = " + cummProb + "\n");
					...
					for (Object term : resAreaScore.testCorpusMapBaseline
							.keySet()) {
						TestTermParams ttp2 = resAreaScore.testCorpusMapBaseline
								.get(term);
					...
					System.out.println("researcherProbBaseline = "
							+ researcherProbBaseline);

					System.out.println("cummresIntPerplexity = "
							+ cummResIntPerProb);
					System.out.println("cummBaselinePerplexity = "
							+ cummBaselinePerProb);
					System.out
							.println("resIntPerplexity = " + resIntPerplexity);
					System.out.println("baselinePerplexity = "
							+ baselinePerplexity);
					System.out.println("finalResIntPerplexity = "
							+ finalResIntPerplexity);
					System.out.println("finlaBaselinePerplexity = "
							+ finalBaselinePerplexity);
					finalResIntPerplexity = 1 / Math.pow(2,
							Math.log(researcherProb) / totFreqTest);
					finalBaselinePerplexity = 1 / Math.pow(2,
							Math.log(researcherProbBaseline) / totFreqTest);

					double logfinalResIntPerplexity = Math.log(researcherProb);
					double logfinalBaselinePerplexity = Math
							.log(researcherProbBaseline);

					System.out.println("logfinalResIntPerplexity = "
							+ logfinalResIntPerplexity);
					System.out.println("logfinlaBaselinePerplexity = "
							+ logfinalBaselinePerplexity);

					System.out.println("logfinalResIntPerplexity/totfreq = "
							+ logfinalResIntPerplexity / totFreqTest);
					System.out.println("logfinlaBaselinePerplexity/totfreq = "
							+ logfinalBaselinePerplexity / totFreqTest);
